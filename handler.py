...
class ActiveDirectory(object):
    ...
    def change_pwd(self, user, current_pwd, new_pwd):
        ...
        if not len(new_pwd) >= status['acct_pwd_policy']['pwd_length_min']:
            msg = 'New password for %s must be at least %d characters, submitted password has only %d.' % (
                user, status['acct_pwd_policy']['pwd_length_min'], len(new_pwd))
        ...
        if status['acct_pwd_policy']['pwd_complexity_enforced']:
            patterns = [r'.*(?P<digit>[0-9]).*', r'.*(?P<lowercase>[a-z]).*', r'.*(?P<uppercase>[A-Z]).*',
                        r'.*(?P<special>[~!@#$%^&*_\-+=`|\\(){}\[\]:;"\'<>,.?/]).*']
            matches = []
            ...
            if len(matches) < 3:
                msg = 'New password for %s must contain 3 of 4 character types (lowercase, uppercase, digit, special), only found %s.' % (
                    user, (', ').join(matches))
                raise self.pwd_vette_failure(user, new_pwd, msg, status)
            ...
            for e in status['user_displayname_tokenized']:
                if len(e) > 2 and e.lower() in new_pwd.lower():
                    msg = 'New password for %s must not contain a word longer than 2 characters from your name in our system (%s), found %s.' % (
                        user, (', ').join(status['user_displayname_tokenized']), e)
                    raise self.pwd_vette_failure(user, new_pwd, msg, status)
        # Encode password and attempt change. If server is unwilling, history is likely fault.
        current_pwd = str('\"' + current_pwd + '\"').encode('utf-16-le')
        new_pwd = str('\"' + new_pwd + '\"').encode('utf-16-le')
        ...
        try:
            ...
            if e[0]['info'].startswith('00000056'):
                ...
                msg = 'New password for %s must not match any of the past %d passwords.' % (
                    user, status['acct_pwd_policy']['pwd_history_depth'])
            raise self.pwd_vette_failure(user, new_pwd, msg, status)
        except ldap.LDAPError as e:
            raise self.ldap_error(e)
    ...
    def set_pwd(self, user, new_pwd):
        ...
        if not len(new_pwd) >= status['acct_pwd_policy']['pwd_length_min']:
            msg = 'New password for %s must be at least %d characters, submitted password has only %d.' % (
                user, status['acct_pwd_policy']['pwd_length_min'], len(new_pwd))
            raise self.pwd_vette_failure(user, new_pwd, msg, status)
        new_pwd = str('\"' + new_pwd + '\"', "iso-8859-1").encode('utf-16-le')
    ...
    def get_user_status(self, user, ou=None):
        ...
        user_status = {'user_dn': '', 'user_id': '', 'user_displayname': '', 'acct_pwd_expiry_enabled': '',
                       'acct_pwd_expiry': '', 'acct_pwd_last_set': '', 'acct_pwd_expired': '', 'acct_pwd_policy': '',
                       'acct_disabled': '', 'acct_locked': '', 'acct_locked_expiry': '', 'acct_expired': '',
                       'acct_expiry': '', 'acct_can_change_pwd': '', 'acct_bad_states': []}
        ...
        if len(results_tmp) != 1:
            for s in range(len(results_tmp)):
                if results_tmp[s][0] != None:
                    results.append(results_tmp[s])
        else:
            result = results_tmp[0][0]
        ...
        s['acct_expired'] = (0 if datetime.datetime.fromtimestamp(s['acct_expiry']) > datetime.datetime.now() or s[
                                                                                                                     'acct_expiry'] == 0 else 1)
        ...
        s['acct_locked_expiry'] = (
            self.ad_time_to_unix(user_attribs['lockoutTime'][0]) + s['acct_pwd_policy']['pwd_lockout_ttl'] if s[
                'acct_locked'] else 0)
    ...
    class authn_failure_time(Exception):
        def __init__(self, user_dn, host):
            self.user_dn = user_dn
            self.host = host
            self.msg = 'user_dn="%s" host="%s" user_dn has time of day login restrictions and cannot login at this time' % (
                user_dn, host)
    ...
    class authn_failure_workstation(Exception):
        def __init__(self, user_dn, host):
            self.user_dn = user_dn
            self.host = host
            self.msg = 'user_dn="%s" host="%s" user_dn has workstation login restrictions and cannot login at this workstation' % (
                user_dn, host)
    ...
    class authn_failure_pwd_expired_admin(Exception):
        def __init__(self, user_dn, host):
            self.user_dn = user_dn
            self.host = host
            self.msg = 'user_dn="%s" host="%s" user_dn\'s password has been administratively expired (force change on next login)' % (
                user_dn, host)
    ...
    class authn_failure_acct_locked(Exception):
        def __init__(self, user_dn, host):
            self.user_dn = user_dn
            self.host = host
            self.msg = 'user_dn="%s" host="%s" user_dn account locked due to excessive authentication failures' % (
                user_dn, host)
    ...
    class user_cannot_change_pwd(Exception):
        def __init__(self, user, status, can_change_pwd_states):
            self.status = status
            self.msg = '%s cannot change password for the following reasons: %s' % (
                user, ', '.join((set(status['acct_bad_states']) - set(can_change_pwd_states))))
